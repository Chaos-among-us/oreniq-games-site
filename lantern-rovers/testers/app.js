...
function clearPrivate(){profile=null;selectedUid='';['messages','ownerMessages','roster','activityRows'].forEach(id=>$(id).replaceChildren());hide('replyForm');}
...
async function loadTester(){
  ...
  if(profile.status==='approved'){
    unsubscribeMessages=onSnapshot(query(collection(db,'testers',uid,'messages'),orderBy('createdAt')),snap=>{if(sameSession(epoch,uid))renderMessages('messages',snap.docs.map(d=>d.data()),uid);},err=>{if(sameSession(epoch,uid))message('messageNotice',errorText(err),true);});
...
function renderMessages(target,items,uid){const box=$(target);box.innerHTML='';if(!items.length){box.innerHTML='<p class="muted small">No messages yet.</p>';return;}for(const m of items){const div=document.createElement('div');div.className='bubble '+((m.role==='tester')?'mine':'');const when=m.createdAt?.toDate?.().toLocaleString()||'Sending…';div.textContent=m.text;const meta=document.createElement('small');meta.textContent=`${m.role==='tester'?'You':'Oreniq games'} · ${when}${m.build?' · '+m.build:''}`;div.append(meta);box.append(div);}box.scrollTop=box.scrollHeight;}
$('messageForm').addEventListener('submit',async e=>{e.preventDefault();if(!profile||profile.status!=='approved')return;const text=$('messageText').value.trim();if(!text)return;try{const id=crypto.randomUUID();await setDoc(doc(db,'testers',currentUser.uid,'messages',id),{role:'tester',text,createdAt:serverTimestamp(),build:$('build').value.trim().slice(0,64),clientId:id});$('messageText').value='';message('messageNotice','Message sent securely.');}catch(err){message('messageNotice',errorText(err),true);}});
...
async function selectTester(t){const epoch=authEpoch,uid=currentUser.uid;selectedUid=t.uid;$('threadTitle').textContent=`${t.alias} · ${t.status}`;$('ownerMessages').innerHTML='';show('replyForm');const msgs=await getDocs(query(collection(db,'testers',t.uid,'messages'),orderBy('createdAt')));if(!sameSession(epoch,uid))return;renderMessages('ownerMessages',msgs.docs.map(d=>d.data()),t.uid);document.querySelectorAll('.person').forEach(el=>el.classList.remove('selected'));}
$('replyForm').addEventListener('submit',async e=>{e.preventDefault();if(!selectedUid)return;const text=$('replyText').value.trim();if(!text)return;try{const id=crypto.randomUUID();await setDoc(doc(db,'testers',selectedUid,'messages',id),{role:'studio',text,createdAt:serverTimestamp(),build:$('replyBuild').value.trim().slice(0,64),clientId:id});$('replyText').value='';await selectTester({uid:selectedUid,alias:$('threadTitle').textContent.split(' · ')[0],status:''});}catch(err){message('globalMessage',errorText(err),true);}});
